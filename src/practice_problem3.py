...
def practice_problem3a(circles):
    ...
    if len(circles) > 0:
        for k in range(len(circles)):
            each_circle = circles[k]
            each_point = each_circle.center
            each_x = each_point.x
            product *= each_x
        return product
    return 1
...
def practice_problem3b(sequence):
    ...
    last_one = sequence[last_index]
    last_type = type(last_one)
    for k in range(last_index):
        if last_type == type(sequence[k]):
            if last_one == sequence[k]:
                return True
    return False
